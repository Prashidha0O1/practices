...
async function saveCookies(page: Page, path: string): Promise<void> {
  const cookies = await page.cookies();
  await fs.writeFile(path, JSON.stringify(cookies, null, 2));
}

async function loadCookies(page: Page, path: string): Promise<void> {
  try {
...
async function CloudflareBypass(): Promise<void> {
